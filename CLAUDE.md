...
- **Migration**: Versioned via a `schema_version` table. `DecisionGraphStorage._migrate_schema()` reads `MAX(version)`, applies any pending steps up to the module-level `CURRENT_SCHEMA_VERSION`, and records each step's `applied_at` timestamp. Migration bodies are idempotent (PRAGMA-checked DDL + `CREATE INDEX IF NOT EXISTS`) so partial / interrupted migrations are safe to re-run.
- **Adding a new migration**: bump `CURRENT_SCHEMA_VERSION` in `decision_graph/storage.py`, add a new branch in `_apply_migration(conn, target)` for the new version number, and ship. The framework handles version tracking, ordering, and idempotency.
- **Reconciliation for early v0.2 users**: if a DB has the v2 threading columns but no `schema_version` rows (because they upgraded before this framework existed), the migration auto-records `[1, 2]` on next launch without re-running ALTER TABLE.
- **Downgrade safety**: if `schema_version` exceeds `CURRENT_SCHEMA_VERSION` (consumer downgraded), the migration logs a warning and proceeds without modifying the DB.
